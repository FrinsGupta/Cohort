...
export const MarketBar = ({ market }: { market: string }) => {
  ...
  // register => I am interested in this data type so now update this data with the callback function after getting update data from websocket
  // deregister => I am no more interested so remove that work from the list(callback function) which is to be called whenever that specific data type update comes through websocket
  useEffect(() => {
    getTicker(market).then(setTicker); // first getting data through http server
    // then establishing below websocket connection
    SignalingManager.getInstance().registerCallback(
      "ticker",
      (data: Partial<Ticker>) =>
        setTicker((prevTicker) => ({
          // when backend return ticker data we set here
